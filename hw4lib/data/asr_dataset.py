...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            if feat_np.shape[1] == self.config['num_feats']:
                feat = feat_np.transpose()
            else:
                feat = feat_np
            ...
            if feat.shape[0] > self.config['num_feats']:
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor([x.size(0) for x in batch_feats], dtype=torch.long) # B
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [x[1] for x in batch] # B x T
            batch_golden       = [x[2] for x in batch] # B x T
            ...
            transcript_lengths = torch.tensor([x.size(0) for x in batch_shifted], dtype=torch.long) # B  
