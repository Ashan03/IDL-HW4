...
def PadMask(padded_input, input_lengths):
    ...
    indices = torch.arange(T, device=padded_input.device).unsqueeze(0)
    lengths = input_lengths.unsqueeze(1).to(padded_input.device)
...
def CausalMask(padded_input):
    ...
    mask_ones = torch.ones((T, T), device=padded_input.device, dtype=torch.bool)

    mask = torch.triu(mask_ones, diagonal= 1)
