...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            logits = self.score_fn(x, **kwargs)
            
            logits = self._apply_repeat_penalty(logits, x, repeat_penalty)
            log_probs = torch.log_softmax(logits / temperature, dim=-1)
            
            token_scores, next_tokens = torch.max(log_probs, dim=-1)
            
            # If finished, don't update score
            scores = torch.where(finished, scores, scores + token_scores)

            x = torch.cat([x, next_tokens.unsqueeze(1)], dim=1)

            is_eos = (next_tokens == getattr(self.tokenizer, 'eos_id', getattr(self.tokenizer, 'eos_token_id', 1)))
    ...
    def generate_beam(
        ...
        batch_size = x.size(0)
        vocab_size = self.tokenizer.vocab_size if hasattr(self.tokenizer, 'vocab_size') else 10000

        beam_scores = torch.zeros((batch_size, beam_width), device=x.device)
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            # Loop over beams to handle score_fn that expects batch_size inputs
            beam_logits_list = []
            for i in range(beam_width):
                beam_input = sequences[:, i, :]
                beam_output = self.score_fn(beam_input, **kwargs)
                beam_logits_list.append(beam_output)
            ...
            if finished.any():
                masked_log_probs = torch.full_like(log_probs, float('-inf'))
                
                eos_id = getattr(self.tokenizer, 'eos_id', getattr(self.tokenizer, 'eos_token_id', 1))
                masked_log_probs[:, :, eos_id] = 0.0

                finished_mask = finished.unsqueeze(-1)
                log_probs = torch.where(finished_mask, masked_log_probs, log_probs)
            
            next_scores = beam_scores.unsqueeze(-1) + log_probs

            next_scores_flat = next_scores.view(batch_size, -1)

            top_scores, top_indices = torch.topk(next_scores_flat, beam_width, dim=1)

            beam_indices = torch.div(top_indices, logits.size(-1), rounding_mode='floor') # Which old beam?
            token_indices = top_indices % logits.size(-1)                                 # Which new token?

            batch_idx = torch.arange(batch_size, device=x.device).unsqueeze(1)
            new_sequences = sequences[batch_idx, beam_indices] # (Batch, Beam, Seq)

            new_tokens = token_indices.unsqueeze(-1)
            sequences = torch.cat([new_sequences, new_tokens], dim=2)

            beam_scores = top_scores

            is_eos = (token_indices == getattr(self.tokenizer, 'eos_id', getattr(self.tokenizer, 'eos_token_id', 1)))
            prev_finished = finished[batch_idx, beam_indices]
