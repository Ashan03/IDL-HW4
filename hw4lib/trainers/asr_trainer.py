...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            targets_shifted = batch[1].to(self.device)
            targets_golden = batch[2].to(self.device)
            feat_lengths = batch[3].to(self.device)
            transcript_lengths = batch[4].to(self.device)
            ...
            # Update progress bar
            current_total_tokens = max(1, total_tokens)
            avg_ce_loss = running_ce_loss / current_total_tokens
            avg_ctc_loss = running_ctc_loss / current_total_tokens
            avg_joint_loss = running_joint_loss / current_total_tokens
            perplexity = torch.exp(torch.tensor(avg_ce_loss))
            
            batch_bar.set_postfix(
                ce_loss=f"{avg_ce_loss:.4f}",
                ctc_loss=f"{avg_ctc_loss:.4f}", 
                loss=f"{avg_joint_loss:.4f}",
                ppl=f"{perplexity:.4f}",
        ...
        avg_ctc_loss = running_ctc_loss / max(1, total_tokens)
        avg_joint_loss = running_joint_loss / max(1, total_tokens)
        avg_perplexity_token = torch.exp(torch.tensor(avg_ce_loss))
        
        chars_per_token = getattr(dataloader.dataset, 'get_avg_chars_per_token', lambda: 1.0)()
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            # Log metrics
            print(f"Train Loss: {train_metrics['joint_loss']:.4f} | Val CER: {val_metrics['cer']:.2f}% | Val WER: {val_metrics['wer']:.2f}%")
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        generator = SequenceGenerator(
            score_fn=self.model,  # Will be set for each batch
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                sos_id = getattr(self.tokenizer, 'sos_token_id', getattr(self.tokenizer, 'sos_id', 0))
